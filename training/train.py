...
NUM_EPISODES = 100  # Reduced for faster runs; increase as needed
GAMMA = 0.99
CLIP_EPS = 0.2
UPDATE_EPOCHS = 4
ENTROPY_COEF = 0.01
DEBUG_PRINT_FREQ = 20  # Print debug every 20 steps
...
try:
    for episode in range(1, NUM_EPISODES + 1):
        ...
        episode_data = {agent: {"obs": [], "actions": [], "log_probs": [], "rewards": [], "values": [], "dones": []} for agent in env.agents}
        ...
        while not all(done.values()):
            ...
            next_obs, rewards, done, truncs, _ = env.step(actions)

            for agent_id in env.agents:
                episode_data[agent_id]["obs"].append(obs[agent_id])
                episode_data[agent_id]["actions"].append(actions.get(agent_id, 0))  # default action 0 if done
                episode_data[agent_id]["log_probs"].append(log_probs.get(agent_id, torch.tensor(0.0)))  # safe default
        ...
        for agent_id, agent in agents.items():
            ...
            actions_tensor = torch.tensor(data["actions"]).to(device)
            old_log_probs = torch.stack(data["log_probs"]).to(device)
            values_tensor = torch.tensor(data["values"]).to(device)
